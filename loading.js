setTimeout(() => {
  ...
  setTimeout(() => {
    ...
    if (!el || el.textContent.trim() !== "tiktok: v3x1.tech") {
    }
  }, 300);
  const el = document.getElementById("lolhaha");
  if (!el || el.textContent.trim() !== "tiktok: v3x1.tech") {
...
document.addEventListener('DOMContentLoaded', () => {
    if (localStorage.getItem('setup') === '1') {
        const setupScreen = document.getElementById('setup_screenid_setup');
        const allApp = document.getElementById('allApp');
        const powerbtn = document.querySelector('.power-button');

        // Immediately hide the setup screen
        if (setupScreen) {
        ...
        if (allApp) {
            allApp.style.display = 'flex';
        }
        
        // Ensure the power button works
        if (powerbtn && typeof powerbtnEvent === 'function') {
            powerbtn.addEventListener('click', powerbtnEvent);
        }

        // Clean up setup-specific event listeners
        if (typeof removeEventListener_setup === 'function') {
