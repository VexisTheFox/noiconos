...
async function addNumber_password(num) {
  ...
  if (input_password.length === 6) {
    const hashedInput = await hashPassword(input_password);
    if (hashedInput === pass_password) {
...
async function handleFullInput_crea_pass() {
  const errorEl = document.getElementById("error_crea_pass");
  errorEl.textContent = "";

  if (stage_crea_pass === 0) {
    const hashedInput = await hashPassword(input_crea_pass);
    if (hashedInput === pass_password) {
      stage_crea_pass = 1;
    ...
    if (input_crea_pass === newPass_crea_pass) {
      pass_password = await hashPassword(newPass_crea_pass);
      hidePopup_open_close(crea_pass);
